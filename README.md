...
- Attempts: 30-min server-authoritative timer, autosave, auto-submit on expiry (+ cron sweep), max 3/cycle then HR approval, rotated random paper per attempt (14 MCQ + 8 FIB + 8 case studies, shuffled per employee). Questions the employee already saw in an earlier attempt this cycle are excluded from later draws — a 42/24/24 bank yields 3 fully disjoint papers (draws top up with repeats only if the bank runs dry). Each paper entry snapshots the question content at draw time — attempts are scored and reviewed against the snapshot, so bank edits never change past attempts (rescore applies current keys deliberately). A certificate always carries the employee's best passing score of the cycle.
