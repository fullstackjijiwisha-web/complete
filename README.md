...
- Attempts: 30-min server-authoritative timer, autosave, auto-submit on expiry (+ cron sweep), max 3/cycle then HR approval, rotated random paper per attempt (12 MCQ + 10 FIB + 4 case studies, shuffled per employee). Each paper entry snapshots the question content at draw time — attempts are scored and reviewed against the snapshot, so bank edits never change past attempts (rescore applies current keys deliberately).
